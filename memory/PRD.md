...
- **New in-app notification "Pending Approval — Workstation Requested"** (`kind: workstation_request_submitted`, auto-seeded, Active by default) fires to **every active Super Admin** whenever a workstation request is created — from the normal-submit path AND the Duplicate-Pending replace path. Body template: *"{{requested_by_name}} submitted a workstation request for {{employee_name}} — seat {{seat_label}} on {{date}}. It is awaiting your approval."* `related_id` points to the new request row and `action_url` deep-links to `/workspace-manager/workstation-requests`. Delivery is best-effort — a notification failure never breaks the submit / replace transaction.
