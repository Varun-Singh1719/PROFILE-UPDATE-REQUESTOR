...
- **"Review : Proposed Plan" dialog now fires for Manual Selection too** (both single-seat + Employee and multi-seat + Team). The Confirm Booking / Submit Request button no longer commits directly — every mode (Auto, Manual + Team, Manual + Employee) goes through the same review step first. `openProposalReview` builds the (seat → employee) assignment from the current state: random pick for Auto, `manualEmpIds` for Manual + Team (Manual allocation), a fresh random for Manual + Team (Random allocation), and `{seat → employeeId}` for Manual + Employee.
- `ConfirmProposalDialog` renders cleanly in single-seat manual mode — the **Team** and **unassigned members** pills are hidden when no team is applicable. The employee-swap picker in the row uses the team pool for Auto / Manual + Team flows and the full active-employee list for Manual + Employee (single seat).
- `handleSave` honors the dialog's edited `explicitAssignment` in every mode, so any last-minute swap or remove inside the review dialog is what gets submitted.
- **New in-app notification "Pending Approval — Workstation Requested"** fires to every active Super Admin on every new workstation request (normal-create path AND Duplicate-Pending replace path).
