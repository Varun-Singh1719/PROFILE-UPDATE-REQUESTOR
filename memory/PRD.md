...
## Sep 06 2026 — ONE-OFF test data: Client Contact Industries back-filled from MySQL projects (NOT an automation)
- `backend/scripts/backfill_contact_industries_from_projects.py` (dry-run by default, `--apply` to write): for each app contact linked via `mysql_ref.mysql_id`, collects distinct (l0_domain, l1_domain, l2_domain, l3_domain) chains from the `mysql_projects` mirror (ids = Domains ids = tree ext_ids), validates each chain against the Infollion tree (local copy of `resolve_paths` rules) and writes `industry_paths` + derived `industries`; legacy placeholder strings dropped; Timeline entries by "System (data migration)". Result: 1,072 contacts updated (0 invalid chains), 192 contacts without MySQL projects untouched. Visible in detail view Industries + Edit form chips. Live users will maintain Industries manually in the form.

## Sep 06 2026 — Activity Summary (contacts) + Client Overview pivot now real MySQL numbers
- `crm_sync._metrics_from` now also returns `activity_by_month = {projects, serviced, calls, revenue: {"YYYY-MM": n}}` (projects/serviced by project receiving_date month; calls/revenue by call_start_time, falling back to the project's receiving month since the mirror's call_start_time is mostly NULL). Written on `client_contacts` by `sync_contacts` and the per-contact Sync (`contact_metrics_for`). Contact detail "Activity Summary" (was all 0) now filled. `month_key()` helper exported.
...
## Sync engine overhaul (MySQL → MongoDB) — follow-up
- **Contact attribution fix (user-approved 1a/2a):** Client Contact Calls/Revenue/last_call now come from the contact's OWN calls (`calls.client_contact`), not from every call on shared projects (which over-counted ~2x). Projects = projects the contact is listed on (`projects.client_contacts`); Serviced = those listed projects with a paid call BY the contact. New `crm_sync._contact_metrics_from`; used by both bulk `compute_all_metrics` and detail `contact_metrics_for`. Client-level metrics unchanged (already exact). `_load_projects_and_calls` now also selects `client_contact_id`.
- **Summaries + error handling:** `sync_clients`/`sync_contacts` return `{processed, updated, created, newly_linked, failed, duplicates}` and continue-on-failure (catch `BulkWriteError`, count failures). Idempotent upsert retained (match by `mysql_ref.mysql_id` → name/email fallback, normalized). `newly_linked` = existing Mongo-only record linked to MySQL for the first time.
- **last_synced_at:** written on every client/contact doc (bulk + detail sync).
- **Endpoints:** `POST /api/crm-sync/run` returns `run_id`; `GET /api/crm-sync/status?run_id=` returns that run. `run_full_sync(run_id=…)` upserts the run doc (no duplicate run rows).
- **Frontend:** `lib/syncStatus.js` (pollSyncCompletion, formatSyncSummary, formatLastSynced). Clients/Client-Contacts list Sync now polls status → "Sync in progress" then "Sync Completed" with summary, then reloads. Detail views show "Last synced …" and a clearer success toast. Same engine powers manual + scheduled sync; scope differs by trigger location.
- **Note:** Mongo-only demo contacts (no MySQL counterpart, e.g. aryan.reddy@mckinsey.com) correctly show 0 after sync (spec #6/#8). Backfilled all 800 clients + 1,072 contacts with corrected numbers + last_synced_at.
